# Jeu Bravo — Synthèse des mécaniques (v8)
...
2. Paiement en P$/PB.
3. **Étape ultime** : évaluation (français, lore, ou les deux) → une note ≥ 60 % (note de passage) garantit l'obtention de la carte en palier `standard` ; la note elle-même détermine ensuite, par un tirage pondéré, une chance d'obtenir un palier de **variante** supérieur (voir section 3, mécanisme révisé — remplace l'ancien système à trois seuils qualitatifs fixes « suffisante/très bonne/excellente », désormais un pourcentage continu).
...
- **Mécanisme révisé (remplace l'ancien système à trois seuils qualitatifs fixes)** : l'obtention de la carte elle-même reste garantie dès que la note de passage (60 %) est atteinte — l'élève reçoit alors au minimum le palier `standard`. Le palier de variante obtenu au-delà du standard se décide par un **tirage pondéré selon le pourcentage exact de la note** (échelle continue de 60 % à 100 %, pas de paliers de note fixes). **Principe de conception retenu** : le hasard n'offre jamais qu'un bonus par rapport à la note obtenue, il ne peut jamais faire perdre ce que l'élève a déjà mérité (le palier `standard` reste acquis quoi qu'il arrive au tirage) — objectif explicite : que tout élève qui passe garde une petite chance d'obtenir les paliers supérieurs, tout en évitant que les meilleurs élèves les obtiennent systématiquement (sinon les cartes de palier 2/3 deviendraient la norme plutôt que l'exception) ;
- **Formule d'interpolation** : progression **géométrique** (et non linéaire) entre la note de passage (60 %) et la note parfaite (100 %) — un ratio constant multiplie la probabilité à chaque point de pourcentage, si bien que p(100 %) / p(60 %) = ×50 pour chacun des quatre paliers concernés. Formule : `p(note) = p60 × (p100 / p60)^((note − 60) / 40)`. Ce choix a été retenu plutôt qu'une interpolation linéaire parce qu'une progression linéaire écraserait la difficulté dans la seconde moitié de l'échelle (60-80 % ferait à peine progresser les chances, puis 80-100 % ferait un bond disproportionné) ; la progression géométrique, elle, monte de façon régulière sur toute la plage tout en concentrant naturellement l'essentiel du gain en valeur absolue vers le segment 90-100 %, ce qui récompense la constance et l'excellence sans jamais rendre le tirage automatique.

**Table de probabilités confirmée (paliers 2 et 3, tirage à l'étape ultime) :**

| Note | Palier 2 — supérieure (argentée) | Palier 3 — suprême (dorée) |
|---|---|---|
| 60 % | 1/100 | 1/500 |
| 70 % | ~1/37 | ~1/187 |
| 80 % | ~1/14 | ~1/70 |
...
- les deux paliers au-delà du suprême (`brillante`, `unique`) ne sont **pas** systématiques — réservés à certaines cartes seulement, jamais une règle automatique. Ils s'obtiennent en deux temps : **(1) admission** — réservée aux élèves ayant reçu du professeur une mention de réussite pour une activité réalisable uniquement en sa présence (accordée depuis sa session « prof » sur Supabase, une fois l'activité accomplie avec lui — aucune autre voie d'admission) ; **(2) tirage** — une fois admis, même mécanisme géométrique que ci-dessus, avec des probabilités nettement plus faibles (voir table ci-dessous). **Principe de conception retenu** (inchangé) : le hasard n'offre jamais qu'un bonus par rapport à la note méritée, il ne peut jamais faire perdre un palier déjà acquis par le mérite — le tirage au hasard ne décide que de l'accès à un palier encore plus rare, jamais d'un recul. Si le plafond de quantité du palier `unique` (section ci-dessous) est déjà atteint au moment du tirage, celui-ci retombe automatiquement au palier `brillante` ;

**Table de probabilités confirmée (paliers 4 et 5, réservés aux élèves admis) :**

| Note | Palier 4 — brillante | Palier 5 — unique |
|---|---|---|
| 60 % | 1/5000 | 1/25 000 |
| 100 % | 1/100 | 1/500 |
...
- Rédiger les premières questions de français/lore pour l'étape ultime de déblocage des cartes, en cohérence avec le mécanisme de tirage pondéré par pourcentage de note (section 2/3, mécanisme révisé).
- Formaliser techniquement (Supabase) la mention de réussite d'activité accordée par le professeur depuis sa session « prof », condition d'admission au tirage des paliers 4/5 (section 3).
- Confirmer si la formule d'interpolation géométrique (section 3) doit être implémentée comme une fonction générique paramétrée par `p60`/`p100` par palier, ou codée en dur carte par carte.
